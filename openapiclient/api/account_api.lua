...
function account_api:account_cart_list(params, exclude, request_from_date, request_to_date, store_url, store_key)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/account.cart.list.json?params=%s&exclude=%s&request_from_date=%s&request_to_date=%s&store_url=%s&store_key=%s",
			self.basePath, http_util.encodeURIComponent(params), http_util.encodeURIComponent(exclude), http_util.encodeURIComponent(request_from_date), http_util.encodeURIComponent(request_to_date), http_util.encodeURIComponent(store_url), http_util.encodeURIComponent(store_key));
	})
...
function account_api:account_failed_webhooks(count, start, ids)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/account.failed_webhooks.json?count=%s&start=%s&ids=%s",
			self.basePath, http_util.encodeURIComponent(count), http_util.encodeURIComponent(start), http_util.encodeURIComponent(ids));
	})
