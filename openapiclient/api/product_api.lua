...
function product_api:product_count(product_ids, since_id, categories_ids, category_id, store_id, lang_id, avail_view, avail_sale, created_from, created_to, modified_from, modified_to, brand_name, product_attributes, status, type, visible, find_value, find_where, report_request_id, return_global, disable_report_cache, use_latest_api_version)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
